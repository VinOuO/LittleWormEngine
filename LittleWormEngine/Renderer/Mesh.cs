...
namespace LittleWormEngine.Renderer
{
    class Mesh
    {
        ...
        public void AddVertices(List<Vertex> _Vertices, List<uint> _Indices)
        {
            ...
            if(_Vertices[0].Normal == null)
            {
                if(_Vertices[0].TexCoord != null)
                {
                    SetVertices_PosTexCord();
                }
                else
                {
                    SetVertices_OnlyPos();
        ...
        public unsafe void SetVertices_PosTexCord()
        {
            float[] _Vertices = new float[Vertices.Count * 5];
            for (int i = 0, _Count = 0; _Count < Vertices.Count; i += 5, _Count++)
            {
                _Vertices[i] = Vertices[_Count].Position.x;
                _Vertices[i + 1] = Vertices[_Count].Position.y;
                _Vertices[i + 2] = Vertices[_Count].Position.z;
            ...
            glVertexAttribPointer(0, 3, GL_FLOAT, false, 5 * sizeof(float), (IntPtr)0);
            glVertexAttribPointer(1, 2, GL_FLOAT, false, 5 * sizeof(float), (IntPtr)(sizeof(float) * 3));
        }
